...
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  // Note: Session validation in background handler would require Firebase initialization
  // For now, we rely on the session token check in the notification service
  // The old device will be logged out anyway, so notifications won't be processed
}
...
class NotificationService extends GetxService {
  ...
  void _onNotificationTapped(NotificationResponse response) {
    ...
    if (response.payload != null && response.payload!.isNotEmpty) {
      ...
      Get.toNamed('/delivery-en-route', arguments: {'orderId': orderId});
    }
  }
  ...
  Future<void> _showLocalNotification({
    ...
    try {
      ...
      await _localNotifications.show(
        ...
        body,
        notificationDetails,
        payload: orderId, // Pass orderId as payload for tap handling
      );
